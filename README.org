...
#+BEGIN_SRC elisp
  (ts-day-name (ts-adjust 'day -2 (ts-now)))             ;=> "Thursday"

  ;; Or, with threading macros:
  (thread-last (ts-now) (ts-adjust 'day -2) ts-day-name) ;=> "Thursday"
  (->> (ts-now) (ts-adjust 'day -2) ts-day-name)         ;=> "Thursday"
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  (list :now (ts-format nil)
        :last-week (ts-format nil (ts-adjust 'day -7 (ts-now))))
